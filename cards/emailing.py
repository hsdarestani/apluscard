...
def send_verification_email(request, user, *, trigger=None):
    ...
    text_body = (
        ...
        "ein Klick genügt, um deine E-Mail-Adresse zu bestätigen:\n"
        f"{url}\n\n"
        f"Der Link ist {EMAIL_VERIFICATION_VALID_DAYS} Tage gültig.\n\n"
        f"{settings.APP_NAME}\n"
    ...
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#17121d">
      ...
      <p>Ein Klick genügt. Danach ist deine digitale Mitgliedskarte bei <strong>{escape(partner_name)}</strong> direkt freigeschaltet.</p>
      <p style="margin:28px 0">
        <a href="{escape(url)}" style="display:inline-block;padding:14px 22px;border-radius:12px;background:#8b35ff;color:#fff;text-decoration:none;font-weight:700">Jetzt bestätigen</a>
      </p>
      <p style="font-size:13px;color:#665d6c">Der Link ist {EMAIL_VERIFICATION_VALID_DAYS} Tage gültig. Falls du dich nicht registriert hast, kannst du diese Nachricht ignorieren.</p>
